...
rendered = {
    ...
    'RELATION (multiple edge types undirected)': qb.reset().match().node().related(label=['Relation', 'OtherRelation'], ref_name='rel').node(),
    'RELATION (multiple edge types forward)': qb.reset().match().node().related_to(label=['Relation', 'OtherRelation'], ref_name='rel').node(),
    'RELATION (multiple edge types backward)': qb.reset().match().node().related_from(label=['Relation', 'OtherRelation'], ref_name='rel').node(),
    ...
    'RELATION (multiple edge types, shortest, min and max)': qb.reset().match().node().related(ref_name='rel', label=['Relation', 'OtherRelation'], min_hops=1, max_hops=2, shortest=True).node(),
