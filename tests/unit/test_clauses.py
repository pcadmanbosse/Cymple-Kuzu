...
rendered = {
    ...
    'NODE': qb.reset().match().node(['labels1', 'labels2'], 'node', {'name': 'Bob'}),
    'NODE MERGE': qb.reset().merge().node(labels=['labels1', 'labels2'], ref_name='n', properties={'name': 'Bob'}).related_to().node(ref_name='m'),
    'NODE (not escaping)': qb.reset().match().node(['labels1', 'labels2'], 'node', {'name': 'Bob'}, escape=False),
    'PATH': qb.reset().match().path(ref_name="p").node(),
    ...
    'RELATION (variable length, with labels)': qb.reset().match().node().related(labels='Relation', ref_name='rel', min_hops=1, max_hops=2).node(),
    'RELATION (multiple edge types undirected)': qb.reset().match().node().related(labels=['Relation', 'OtherRelation'], ref_name='rel').node(),
    'RELATION (multiple edge types forward)': qb.reset().match().node().related_to(labels=['Relation', 'OtherRelation'], ref_name='rel').node(),
    'RELATION (multiple edge types backward)': qb.reset().match().node().related_from(labels=['Relation', 'OtherRelation'], ref_name='rel').node(),
    'RELATION (SHORTEST)': qb.reset().match().node().related(ref_name='rel', min_hops=-1, max_hops=-1, shortest=True).node(),
    'RELATION (SHORTEST min and max)': qb.reset().match().node().related(ref_name='rel', min_hops=1, max_hops=2, shortest=True).node(),
    'RELATION (ALL SHORTEST min and max)': qb.reset().match().node().related(ref_name='rel', min_hops=1, max_hops=2, shortest='all').node(),
    'RELATION (SHORTEST no min nor max)': qb.reset().match().node().related(ref_name='rel', shortest=True).node(),
    'RELATION (multiple edge types, shortest, min and max)': qb.reset().match().node().related(ref_name='rel', labels=['Relation', 'OtherRelation'], min_hops=1, max_hops=2, shortest=True).node(),
    ...
    'YIELD': qb.reset().call().procedure("db.labels()").yield_(("labels", "labels")).where("labels", "CONTAINS", "User").return_literal("count(labels) AS numlabels"),
    'YIELD (list)': qb.reset().call().procedure("db.labels()").yield_([('length(labels)', 'len'), ('count(labels)', 'cnt')]),
...
expected = {
    ...
    'MERGE': 'MERGE',
    'NODE': 'MATCH (node: labels1: labels2 {name : "Bob"})',
    'NODE MERGE': 'MERGE (n: labels1: labels2 {name : "Bob"})-->(m)',
    'NODE (not escaping)': 'MATCH (node: labels1: labels2 {name : Bob})',
    'PATH':'MATCH p = ()',
    ...
    'RELATION (variable length, with labels)': 'MATCH ()-[rel: Relation*1..2]-()',
    'RELATION (multiple edge types undirected)': 'MATCH ()-[rel:Relation|:OtherRelation]-()',
    'RELATION (multiple edge types forward)': 'MATCH ()-[rel:Relation|:OtherRelation]->()',
    'RELATION (multiple edge types backward)': 'MATCH ()<-[rel:Relation|:OtherRelation]-()',
    'RELATION (SHORTEST)': 'MATCH ()-[rel*SHORTEST ]-()',
    'RELATION (SHORTEST min and max)': 'MATCH ()-[rel*SHORTEST 1..2]-()',
    'RELATION (ALL SHORTEST min and max)': 'MATCH ()-[rel*ALL SHORTEST 1..2]-()',
    'RELATION (SHORTEST no min nor max)': 'MATCH ()-[rel]-()',
    'RELATION (multiple edge types, shortest, min and max)': 'MATCH ()-[rel:Relation|:OtherRelation*SHORTEST 1..2]-()',
    ...
    'YIELD': 'CALL db.labels() YIELD labels AS labels WHERE labels CONTAINS "User" RETURN count(labels) AS numlabels',
    'YIELD (list)': 'CALL db.labels() YIELD length(labels) AS len, count(labels) AS cnt',
